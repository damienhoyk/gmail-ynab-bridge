...
if [ -z "$STAGED_FILES" ]; then
    ...
    DOMAIN_COUNT=$(echo "$STAGED_FILES" | rg "/domain/" | wc -l | xargs)
    INFRA_COUNT=$(echo "$STAGED_FILES" | rg "(-client|-dynamodb|-handler|-event-handler)/" | wc -l | xargs)
    TEST_COUNT=$(echo "$STAGED_FILES" | rg "/test/" | wc -l | xargs)
    ...
    echo -e "\n### Kotlin TODO Check ###"
    TODO_LIST=$(echo "$STAGED_FILES" | rg "\\.kt$" | xargs -r rg -n "TODO|FIXME")
    echo "${TODO_LIST:-none}"

    echo -e "\n### Debt Check ###"
    MANUAL_LOGGING=$(echo "$STAGED_FILES" | rg "\\.kt$" | xargs -r rg "println\\(|System\\.out\\." | wc -l | xargs)
...
$GIT log -n 5 --pretty=format:"%h %as %an: %s" --compact-summary
