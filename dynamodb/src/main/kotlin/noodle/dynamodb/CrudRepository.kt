...
abstract class CrudRepository : Crud() {

    abstract val name: String
    abstract val partitionKey: String

    suspend fun put(partition: String, block: Item.() -> Unit = {}) = put(key(partition), block)
    suspend fun get(partition: String) = get(key(partition))
    suspend fun update(partition: String, block: Item.() -> Unit = {}) = update(key(partition), block)
