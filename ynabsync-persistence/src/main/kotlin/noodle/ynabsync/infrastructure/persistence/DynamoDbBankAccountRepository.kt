...
public class DynamoDbBankAccountRepository(
     ...
     *
     * Builds the partition URI `//<email>/account/<number>`, queries the
     * DynamoDB table, and parses each `sort` value into a [BankAccount]. Sort keys that
     * are absent, malformed, or fail validation (scheme-relative form `//…`, host `app.ynab.com`
     * — case-insensitive — non-blank userInfo, and a `/budget/<id>/account/<id>` path) are
     * logged and skipped.
     */
    override suspend fun getAccounts(
        ...
        val partition = "//$email/account/$number"
     ...
     *
     * Builds the partition URI `//<email>/account/<number>` and writes
     * an incomplete sort key `//<userId>@app.ynab.com` (no path).
